...
const GamesCarousel = ({url})=>{
    ...
    return (
        <div className="game-carousel">
            <button className="carousel-button carousel-left" onClick={()=> setShift(prev => prev -= 1)}>
            ...
            games?.results.map((game, i)=>{
                ...
                return (
                    <GameInfoCard
                        ...
                        key={game.id}
                        id={game.id}
                        carouselClass = {shiftedI > 4 ? "hidden" : `game-info-card-${shiftedI}`}
                    />)
            })}
            <button className="carousel-button carousel-right" onClick={()=> setShift(prev => prev -= 1)}>
